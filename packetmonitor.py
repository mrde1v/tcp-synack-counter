...
def capture_stats(interface, local_ip, port=None):
    ip_stats_per_second = defaultdict(lambda: {"syn_count": 0, "ack_count": 0, "conn_count": 0})
    ip_stats_overall = defaultdict(lambda: {"syn_count": 0, "ack_count": 0, "conn_count": 0})
    ...
    def packet_callback(packet):
        if IP in packet and TCP in packet:
            ...
            if port is not None and tcp.dport != port:
                return
            
            ip_stats_per_second[ip_src]["conn_count"] += 1 if tcp.flags & 2 else 0  # CONN init
            ...
            ip_stats_overall[ip_src]["conn_count"] += 1 if tcp.flags & 2 else 0
            ip_stats_overall[ip_src]["syn_count"] += 1 if tcp.flags & 2 else 0
            ip_stats_overall[ip_src]["ack_count"] += 1 if tcp.flags & 16 else 0
            
            ip_counts[ip_src] += 1
            
            print("IP:", ip_src, "SYN:", ip_stats_per_second[ip_src]["syn_count"], "ACK:", ip_stats_per_second[ip_src]["ack_count"], "CONN:", ip_stats_per_second[ip_src]["conn_count"])
...
def save_per_second_stats(ip_stats_per_second, output_file):
    with open(output_file, "a") as f:
        for ip, stats in ip_stats_per_second.items():
            f.write(f"{ip}: SYN={stats['syn_count']}, ACK={stats['ack_count']}, CONN={stats['conn_count']}\n")
...
def main():
    ...
    parser.add_argument("--port", type=int, help="Destination port to monitor")
    ...
    if args.port:
        print("Monitoring packets to port:", args.port)
    ...
    while True:
        ip_stats_per_second, _, ip_counts = capture_stats(args.interface, local_ip, args.port)
